...
import torch
import numpy as np
from dngs.utils.general_utils import inverse_sigmoid, get_expon_lr_func, build_rotation
from torch import nn
import os
from dngs.utils.system_utils import mkdir_p
from plyfile import PlyData, PlyElement
from dngs.utils.sh_utils import RGB2SH
from dngs_simple_knn._C import distCUDA2
from dngs.utils.graphics_utils import BasicPointCloud
from dngs.utils.general_utils import strip_symmetric, build_scaling_rotation, quaternion_to_matrix
from dngs.scene.neural_renderer import GridRenderer
...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        dist2 = torch.clamp_min(
            distCUDA2(torch.from_numpy(np.asarray(pcd.points)).float().cuda())[0],
            0.0000001,
        )
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_features, new_opacities, new_scaling, new_rotation, new_depth_err):
        d = {"xyz": new_xyz,
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size, opa_thresh, max_dist):
        if max_dist is not None:
            outliers_mask = torch.sqrt(
                distCUDA2(self._xyz.detach())[0].view(max_dist.shape)
            ) > max_dist
