...
import torch
import math
from dngs_diff_gaussian_rasterization import GaussianRasterizationSettings, GaussianRasterizer
...
from dngs.utils.sh_utils import eval_sh
...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, inference=False):
    ...
    confidence = getattr(pc, "confidence", None)
    if confidence is None:
        confidence = torch.ones((pc.get_xyz.shape[0], 1), device=pc.get_xyz.device)
